...
parser.add_argument('--test_interval', type=int, default=5,  help='how many epochs to wait before another test (default = 1)')
...
parser.add_argument('--grad_scale_factor', default=4)
...
model_save_dir = r'../model_files/vgg8_noisy_trn.pth'
...
# out = open(std_NS_output_dir+"PythonWrapper_Output.csv",'ab')
...
# delta_distribution = open(std_NS_output_dir+"delta_dist.csv",'ab')
...
# weight_distribution = open(std_NS_output_dir+"weight_dist.csv",'ab')
...
# model = resnet.resnet44(args = args, logger=logger)
print(f'model: \n{model}')
# exit()
if args.cuda:
    model.cuda()
    ...
    for epoch in range(args.epochs):
        ...
        if epoch % args.test_interval == 0:
            ...
            if acc > best_acc:
                ...
                misc.model_save(model, model_save_dir, old_file=old_file, verbose=True)
                best_acc = acc
                # old_file = new_file
