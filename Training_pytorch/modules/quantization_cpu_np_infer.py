...
class QConv2d(nn.Conv2d):
    ...
    def forward(self, input):
        ...
        if self.inference == 2:
            # print(f'np_infer called: inf={self.inference}')
            weight1 = self.weight * self.scale + (self.weight - self.weight * self.scale).detach()
            weight = weight1 + (wage_quantizer.Q_onoff(weight1,self.wl_weight, self.onoffratio) -weight1).detach()
                    ...
                    if numSubArray == 0:
                        ...
                        if weight.shape[1] == 3:
                            ...
                            for k in range (int(bitWeight/self.cellBit)):
                                ...
                                # print(f'remainderQ.size(): {remainderQ.size()}')
                                # print(f'reminderQ.size() data type: {type(remainderQ.size())}')
                                # exit(0)
