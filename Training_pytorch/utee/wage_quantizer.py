...
def Q_onoff(x, bits, onoff):
    '''
    x supposed to be weight, bits supposed to be bitWeight [8.19]
    '''
    assert bits != -1
    if bits==1:
        return torch.sign(x)
    ...
    delta = 1/S(bits)
    upper = 1 - delta
    lower = -1 + delta + (2-2*delta)/onoff

    delta_weight = (upper - lower) / (2**bits - 2)
    x_q = torch.round( (x - (-1+delta)) / delta ) * delta_weight + lower
    return x_q
...
def QE(x, bits):
    x_old = x
    max_entry = x.abs().max()
    # assert max_entry != 0, "QE blow"  # 2)
    #if max_entry != 0:
    if max_entry == 0:                  # 2) to circumvent all zero cases
        # print('[WARNING]: QE blow')
        return torch.zeros_like(x)    # 2)
    x /= shift(max_entry)   
    return Q(C(x, bits), bits)

# modified QG. Original QG refer to github
...
## modified QG (no non-ideality)
def QG_ideal(x, bits_G, lr):
    max_entry = x.abs().max()
    # assert max_entry != 0, "QG blow"
    if max_entry == 0:                                                      # 2)
        # print(f'WARNING, maximum in QG x is 0, returning zero gradient')    # 2)
        return torch.zeros_like(x)                                     # 2)
    x /= shift(max_entry)
    norm = lr * x
    norm = SR(norm)
