...
def Neural_Sim(self, input, output):
    ...
    input_activity = open('./std_NS_output/input_activity.csv', "a")
...
def stretch_input(input_matrix,window_size = 5,padding=(0,0),stride=(1,1)):
    '''
    modified by zkf[8.14]
    '''
    input_shape = input_matrix.shape
    # print(f'input_shape:{input_shape}, padding = {padding}, stride = {stride}, window_size: {window_size}')

    item_num = int((input_shape[2] + 2*padding[0] - window_size) / stride[0] + 1) * int((input_shape[3] + 2*padding[1] - window_size) / stride[1] + 1)  # 2)
    output_matrix = np.zeros((input_shape[0],int(item_num),input_shape[1]*window_size*window_size))
    ...
    for i in range(int( (input_shape[2] + 2*padding[0] - window_size) / stride[0] + 1 )):
        for j in range(int( (input_shape[3] + 2*padding[1] - window_size) / stride[1]  + 1 )):
            ...
            iter += 1 # corresponds to item_num 2)
...
def hardware_evaluation(model,wl_weight,wl_activation,numEpoch):
    ...
    ## Original function arranged model into two parts: features and classifier
    ## which is not quite universal. 
