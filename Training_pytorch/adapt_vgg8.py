...
The first adaptation script. Mainly used for adapting VGG8 network
...
# args.epochs = 3               # tuning epoch?
args.mode = 'WAGE'            # (== 'WAGE') apply WAGE quantization mode
args.grad_scale = 1/8
# args.onoffratio = 1000
# args.inference=0
args.decreasing_lr = '4'
args.batch_size = 200       # tuning batch size?
...
model_pth = r"../model_files/VGG8_seed.pth"     # VGG8 trained on cifar10

use_pretrain = None
use_pretrain = model_pth
model = model.cifar10(args = args, logger=logger, pretrained = use_pretrain)                      # VGG8
...
print(f' --- config: \
        ...
        decrease_lr: {args.decreasing_lr}, \
        initial grad_scale: {args.grad_scale}, \
        grad_scale factor:{args.grad_scale_factor}, \
        gamma: {gamma}, \
        alpha: {alpha}, \
        adapt_idx: {args.adapt_idx}, \
        onoff: {args.onoffratio}, \
        num_sample: {num_sample}, \
        check_overfit: {args.check_overfit}, \
...
try:
    ...
    for epoch in range(args.epochs):
        ...
        # weight and gradient check out (disabled) 
        for name, param in list(model.named_parameters()):
            oldWeight[k] = param.data + param.grad.data
            # print(f'layer: {name}\ngrad mean: {param.grad.data.mean()}, grad std: {param.grad.data.std()}')
            k = k+1
        ...
        if epoch % args.test_interval == 0:
            ...
            if args.check_overfit == 1:
                test_loss = 0
                correct = 0
                logger("Check overfit phase (inf on training data)")
                # for i, (data, target) in enumerate(test_set):
                for i in range(len(train_data)):                             # cifar10-c
                    data, target = train_data[i], train_target[i]               # cifar10-c
                    target = target.long()
                    # if i==0:
                ...
                test_loss = test_loss / len(train_data) # average over number of mini-batch
                acc = 100. * correct / (len(train_data) * args.batch_size)
                print(f'correct: {correct}, len(train_data): {len(train_data)}, batch size: {args.batch_size}')
                # logger('\tEpoch {} Test set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)'.format(
                #     epoch, test_loss, correct, len(test_data)*args.batch_size, acc))
                print('Epoch {} inf on train set: Average loss: {:.4f}, Accuracy: {}/{} ({:.0f}%)\n'.format(
                    epoch, test_loss, correct, len(train_data)*args.batch_size, acc))
            ...
            ## saving python wrapper output (disabled)
            # np.savetxt(out, [[epoch, test_loss.cpu().numpy(), accuracy]], delimiter=",",fmt='%f')
            
            if acc > best_acc:
                new_file = os.path.join(args.logdir, 'best-{}.pth'.format(epoch))
                misc.model_save(model, save_dir, old_file=old_file, verbose=False)
                best_acc = acc
                old_file = new_file
