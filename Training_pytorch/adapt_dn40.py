...
block_names = [['conv1', 'dense1', 'trans1'],
               ['dense2',  'trans2'],
               ['dense3'],
               ['classifier']
            ]
...
model_pth = r"../model_files/DenseNet40_seed.pth"

use_pretrain = None
use_pretrain = model_pth
model = DenseNet.densenet40(args = args, logger=logger, pretrained = use_pretrain)				# DenseNet40
...
try:
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx in range(len(train_data)):
            ...
            # loss = wage_util.softmax_entropy(output).mean(0)    # loss: unsupervised training
            ...
            ## checking grad.data status
            if epoch == 0 and batch_idx == 0:
                for name, param in list(model.named_parameters())[::-1]:
                    print(f' ---- layer: {name} ----')
                    print(f'grad: \n\tavg {torch.mean(param.grad.data)}\n\tvariance: {torch.var(param.grad.data)}')
            # exit()
            optimizer.step()
