...
# from L1BN import L1BatchNorm, QL1BN
name=0
...
def conv3x3(in_planes, out_planes, stride=1, groups=1, dilation=1,args=None,logger=None):
    ...
    global name
    if args.mode == "WAGE":       
        conv2d = QConv2d(in_planes, out_planes, kernel_size=3, stride=stride, padding=dilation, logger=logger,wl_input = args.wl_activate,wl_activate=args.wl_activate,
...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None, groups=1,
        ...
        # print(f'BasicBlock: args: {args.inference}')
        ...
        self.bn1 = norm_layer(planes)                     # 2) original BN, either L1 or L2

        self.relu = nn.ReLU(inplace=True)
        self.conv2 = conv3x3(planes, planes,args=args,logger=logger)
        self.bn2 = norm_layer(planes)                     # 2) original BN, either L1 or L2
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, args, logger, num_classes=10, zero_init_residual=False,
        ...
        # print(f'--ResNet: block.args = {block.args}')
        ...
        self.features = nn.Sequential(self.conv1, self.bn1, self.relu,
         *self.layer1, *self.layer2, *self.layer3, self.avgpool)
    ...
    def _make_layer(self, block, planes, blocks, stride=1, dilate=False, args=None, logger=None):
        ...
        if stride != 1 or self.inplanes != planes * block.expansion:           
            downsample = nn.Sequential(
                ...
                norm_layer(planes * block.expansion),      # FP 
            )
...
def _resnet(arch, block, layers, pretrained=None, progress=True, args=None, logger=None, **kwargs):
    ...
    print(f'\t --- Resnet Model. Pretrain = {pretrained}')
    if pretrained==True:
        state_dict = load_state_dict_from_url(model_urls[arch],
                                              progress=progress)
        model.load_state_dict(state_dict, strict=False)
    elif pretrained is not None:
        pth = torch.load(pretrained)            # 2)
        # del pth['fc.bias']                      # 2)
        model.load_state_dict(pth)              # 2)
        print(f'loading .th succeeded \npth file : {pretrained}')
        # model.load_state_dict(torch.load(pretrained))
    return model

def resnet20(pretrained=None, progress=True, args=None, logger=None, **kwargs):
    r"""
    Resnet20, for Cifar10 [zkf[8.13]]
    ...
    """
    # _BasicBlock = BasicBlock(args = args)   block=_
    return _resnet('resnet18', BasicBlock, layers = [3, 3, 3], pretrained = pretrained, progress =progress, args=args, logger=logger,
                   **kwargs)

def resnet44(pretrained=None, progress=True, args=None, logger=None, **kwargs):
    return _resnet('resnet18', BasicBlock, layers = [7, 7, 7], pretrained = pretrained, progress =progress, args=args, logger=logger,
                   **kwargs)
